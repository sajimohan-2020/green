<script lang="ts">
  ...
  function scrollHiddenCards(direction: "left" | "right") {
    ...
    const maxScroll =
      hiddenCardsContainer.scrollWidth -
      hiddenCardsContainer.clientWidth;
...
<section class="bg-[#F4F9FF] flex flex-col gap-5 p-4 lg:p-0 mt-20">
  <div class="flex flex-col lg:flex-row gap-5 px-4 xl:px-9">
    ...
    <!-- RIGHT CARD SLIDER -->
    <div class="lg:relative lg:w-[60%] xl:w-[65%] w-full lg:mr-5 lg:mt-16 mt-8 md:ml-[1.1rem] lg:ml-10">
      <div
      ...
      >
        ...
        >
          <p class="text-4xl lg:text-6xl text-[#2D8AE7] font-[700]">{card.cardNo}</p>
          <p class="text-lg lg:text-[20px] text-[#1E3A8A] font-[500] py-4 text-wrap">{card.cardtitle}</p>
          ...
          <ul class="list-disc pl-4 text-sm leading-relaxed space-y-1 m-4">
      ...
      <!-- Mobile Navigation -->
      <div class="flex lg:hidden justify-center items-center gap-6 mt-6 pb-8 mb-4">
        <button
          class="bg-[#2D8AE7] text-white rounded-full h-16 w-16 p-2 hover:bg-blue-600 transition-colors flex items-center justify-center"
          on:click={() => scrollHiddenCards("left")}
          aria-label="Scroll left"
